...
def record_metric_snapshot(conn: sqlite3.Connection, records: list, timestamp: str | None = None) -> None:
    """Record summary metric counts for all states, components, and consolidated."""
    ...
    def calc(subset):
        tracked = len(subset)
        expired = sum(1 for r in subset if r["days"] < 0)
        critical = sum(1 for r in subset if 0 <= r["days"] <= 15)
        warning = sum(1 for r in subset if 15 < r["days"] <= 30)
        healthy = sum(1 for r in subset if r["days"] > 30)
        future = [r["days"] for r in subset if r["days"] >= 0]
        soonest = min(future) if future else (min((r["days"] for r in subset), default=0) if subset else 0)
        return tracked, expired, critical, warning, healthy, soonest

    # Consolidated snapshot
    t, e, c, w, h, s = calc(records)
    conn.execute(
...
def ensure_metric_snapshots(conn: sqlite3.Connection, records: list) -> None:
    ...
    # Seed 8 weekly historical snapshots showing realistic operational progress leading to today
    today = date.today()
    base_t = len(records)
    ...
    history_deltas = [
        # (weeks_ago, e_delta, c_delta, w_delta, h_delta)
        (7, +2, +1, +3, -6),
        (6, +2, +1, +2, -5),
        (5, +1, +2, +2, -5),
        (4, +1, +1, +1, -3),
